...
package io.cdap.nlp.directive;

import io.cdap.cdap.api.annotation.Description;
import io.cdap.cdap.api.annotation.Name;
import io.cdap.cdap.api.annotation.Plugin;
import io.cdap.nlp.EntityAnalysisService;
...
/**
 * This class <code>SentimentAnalysis</code> redacts sensitive data from a column of text.
...
@Name(EntityAnalysis.NAME)
@Categories(categories = { "nlp", "classify"})
@Description("Detects categories in text")
public class EntityAnalysis implements Directive {
  private static final Logger LOG = LoggerFactory.getLogger(EntityAnalysis.class);
  ...
  public UsageDefinition define() {
    ...
    builder.define("lang", TokenType.IDENTIFIER);
  ...
  public void initialize(Arguments args) throws DirectiveParseException {
    ...
    try {
      ...
      service = new EntityAnalysisService(provider.getClient(), provider.getProject());
  ...
  public List<Row> execute(List<Row> rows, ExecutorContext ctx)
    ...
    for (Row row : rows) {
      ...
      if (value instanceof String) {
        Row result = service.getResult((String) row.getValue(idx));
        for (Pair<String, Object> field : result.getFields()) {
          row.add(String.format("%s_%s", column.value(), field.getFirst()), field.getSecond());
        }
      } else {
        throw new DirectiveExecutionException(
          String.format("Entity analysis can only be applied to a text field")
        );
      }
    }
    return rows;
